...
def create_nontelem_features(data, hole_id_col, drilltime_col, operator_col, redrill_col, hole_depth_col):
    # Group by hole IDs
    ...
    features = hole_grps.agg({drilltime_col: 'mean',
                              redrill_col:'max',
                              operator_col: 'first'}).rename(columns=feat_names)
...
def calc_penetration_rate(depth_telem, feature_df, hole_id_col):
    ...
    # Filter out no longer relevant columns
    exclude_cols = ['min', 'max']
...
if len(sys.argv) == 3:
    ...
    nontelem_feats = create_nontelem_features(df, 'Unique_HoleID_y', 'DrillTime', 'FirstName', 'Redrill', 'Hole Depth')

    # Output calculated features to file
    nontelem_feats.to_csv(output_file_path, index=False)

    print('Non-telemetry features calculated and written to file')
