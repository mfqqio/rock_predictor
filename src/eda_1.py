...
eda_data = pd.read_csv(join_path)
print('telemetry table dimensions:', eta_data.shape)


# Run a detailed summary of the df
eda_data.describe()

# Get information about each column
eda_data.info()

# Number of records for each field status type
# 1 = Down
# 2 = Ready
# 3 = Standby
# 4 = Delay
eda_data.groupby(['FieldStatus']).agg(['count'])['FieldData']

# Look at distribution of drill time
# Distribution of drill time
num_bins = 50
plt.hist(eda_data['DrillTime'], num_bins, normed=1, facecolor='blue', alpha=0.5)
plt.show()
